...
test('backend creates missing monthly shard then reads it', async()=>{
  ...
  const fetchImpl=async(url,options={})=>{const method=options.method||'GET';calls.push({url,method,body:options.body,headers:options.headers});if(method==='GET'&&!created)return response(404,{error:'missing'});if(method==='POST'){created=true;return response(200,{success:true})}return response(200,{})};
  ...
  assert.equal(calls[0].headers['X-Mantle-Key'],'test-key');
...
test('backend patches parking and room records in matching month', async()=>{
  const calls=[];
  const fetchImpl=async(url,options={})=>{calls.push({url,method:options.method||'GET',body:options.body,headers:options.headers});return response(200,{})};
  const backend=new ParkingBackend({baseUrl:'https://example.test/v2',namespace:'parking',key:'test-key',fetchImpl});
